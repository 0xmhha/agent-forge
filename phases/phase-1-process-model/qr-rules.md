...
2. 도메인 프로필 체크리스트 생성 (프로필이 있는 경우)
   - forbidden_patterns → 위반 여부 검사 항목으로 변환
   - required_patterns → 존재 여부 검사 항목으로 변환
   - priority_matrix → 심각도 가중치 조정 (integration-guide.md §3 참조)
...
4. 규칙 적용
   ...
   - 도메인 프로필 체크리스트 항목은 해당 severity로 판정
   - priority_matrix 가중치로 심각도 조정 (integration-guide.md §3):
     · 0.9~1.0 → SHOULD를 MUST로 승격 가능
     · 0.3~0.5 → SHOULD를 COULD로 강등 가능
     · 0.0~0.2 → 관련 검사 생략 가능

5. 이중 경로 검증 (MUST에만 적용)
   - 경로 A: "이것이 문제인 이유는?"
   - 경로 B: "이것이 문제가 아닌 이유는?"
