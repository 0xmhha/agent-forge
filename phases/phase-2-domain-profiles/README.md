...
- [x] 우선순위 매트릭스 필드 확정
- [x] 규약 오버라이드 옵션 확정
- [x] 도메인 지식 구조 확정 (용어, 금지, 필수)
- [x] 워크플로우 커스터마이징 옵션 확정
- [x] 스키마 검증 규칙 (필수 필드, 유효 범위)

### 2.2 예시 프로필 3개
- [x] `profiles/fintech.yaml` — 규정 준수 최우선
- [x] `profiles/game-dev.yaml` — 성능 최우선
- [x] `profiles/startup-mvp.yaml` — 속도 최우선

### 2.3 통합 메커니즘 설계
- [x] 프로필이 컨벤션 계층 Tier 1.5에 삽입되는 방식
- [x] 에이전트가 프로필을 읽는 시점과 방법
- [x] 프로필 내 forbidden_patterns가 QR MUST로 변환되는 로직
- [x] 프로필 내 priority_matrix가 심각도 가중치에 미치는 영향
...
- [x] 3개 이상의 도메인 프로필이 스키마에 맞게 작성
- [x] 동일 코드에 다른 프로필 적용 시 다른 리뷰 결과가 나오는 것을 시연
